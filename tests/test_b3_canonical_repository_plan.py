...
class TestCanonicalRepositoryPlan(unittest.TestCase):
    ...
    def test_literal_repository_url_token_count_and_location(self):
        ...
        for row in range(3):
            for cell in (0, 2, 3, 4):
                with self.subTest(row=row, cell=cell, failure="count"):
                ...
                with self.subTest(row=row, cell=cell, failure="location"):
                    cells = self._cells(row)
    ...
    def test_multiple_semantic_failures_are_sorted_and_unique(self):
        lines = self.lines.copy()
